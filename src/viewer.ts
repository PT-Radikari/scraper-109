...
import { loadHidden, saveHidden, updateHidden, HiddenKind } from "./dashboardHidden";
import {
  loadDashboardConfig,
  getPortalSummaries,
  getRuns,
...
// a row back.
app.post("/api/dashboard/hide", express.json(), async (req, res) => {
  const config = loadDashboardConfig();
  if (!config) {
    res.status(503).json({ error: "Supabase not configured (SCORING_SUPABASE_URL/ANON_KEY missing)" });
    return;
  ...
  const ids: number[] = Array.isArray(req.body?.ids)
    ? req.body.ids.filter((id: unknown): id is number => Number.isInteger(id) && (id as number) > 0)
    : [];
  ...
  try {
    const next = updateHidden(await loadHidden(config), kind as HiddenKind, ids, action);
    await saveHidden(config, next);
    res.json({ kind, action, count: ids.length, hiddenTotal: next[kind as HiddenKind].length });
  } catch (error) {
    dashboardError(res, error);
  }
});
...
app.get("/api/dashboard/vacancies", async (req, res) => {
  ...
  try {
    ...
    res.json(await getVacancies(config, { portal, search, ...visibility }));
  } catch (error) {
    dashboardError(res, error);
  }
});
  ...
  try {
    ...
    res.json(
      await getCandidates(config, {
        portal,
        search,
        hasCv: presence(req.query.cv),
        hasPhone: presence(req.query.phone),
