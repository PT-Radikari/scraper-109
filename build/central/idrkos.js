 ...
 */
function newCandidateResult(source) {
    return {
        matched: false,
        idrkos_staf_id: null,
        status: "scraped_new",
        match_field: null,
        listing_priority: types_1.LISTING_PRIORITY.scraped_new,
        source,
    };
}
/**
...
class IdrkosService {
     ...
     */
    constructor(config, supabase = new supabaseClient_1.SupabaseRestClient(config), transport = new supabaseClient_1.AxiosHttpTransport()) {
        this.config = config;
    ...
    crossCheckCandidate(candidate) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (mode === "rpc" || mode === "auto") {
                const viaRpc = yield this.crossCheckViaRpc(identity);
                if (viaRpc)
                    return viaRpc;
                if (mode === "rpc")
                    return newCandidateResult("none");
            }
            const viaApi = yield this.crossCheckViaApi(identity);
            if (viaApi)
                return viaApi;
            return newCandidateResult("none");
        });
     ...
     */
    crossCheckViaRpc(identity) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this.supabase.isConfigured())
                return null;
            try {
                const payload = yield this.supabase.rpc("cross_check_idrkos_candidate", {
                ...
                // path in pickTalent.
                if (row.match_field === "name")
                    return newCandidateResult("rpc");
                return this.toResult(Boolean(row.matched && row.idrkos_staf_id), row.idrkos_staf_id ? String(row.idrkos_staf_id) : null, row.match_field || null, "rpc");
            }
            catch (error) {
                console.warn("IDRKOS cross-check via RPC failed, falling back:", error.message);
                return null;
            }
        });
    }
     ...
     */
    crossCheckViaApi(identity) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a;
            if (!this.config.idrkosBaseUrl)
                return null;
            const lookups = [];
            if (identity.nik)
                lookups.push({ field: "nik", value: identity.nik });
            if (identity.email)
                lookups.push({ field: "email", value: identity.email });
            if (identity.phone)
                lookups.push({ field: "phone", value: identity.phone });
            ...
            try {
                for (const lookup of lookups) {
                    const response = yield this.transport.request({
                        ...
                        headers,
                        params: { [lookup.field]: lookup.value },
                        timeout: this.config.requestTimeoutMs,
                    });
                    const talent = this.pickTalent(response.data, identity, lookup.field);
                    if (talent) {
                        const stafId = (_a = talent.staf_id) !== null && _a !== void 0 ? _a : talent.id;
                        return this.toResult(stafId !== undefined && stafId !== null, stafId !== undefined && stafId !== null ? String(stafId) : null, lookup.field, "api");
                    }
                }
                return newCandidateResult("api");
            }
            catch (error) {
                console.warn("IDRKOS cross-check via /talents API failed:", error.message);
                return null;
            }
        });
    }
     ...
     */
    pickTalent(body, identity, field) {
        const envelope = body;
        const raw = Array.isArray(body) ? body
            : Array.isArray(envelope === null || envelope === void 0 ? void 0 : envelope.data) ? envelope.data
                : Array.isArray(envelope === null || envelope === void 0 ? void 0 : envelope.talents) ? envelope.talents
                    : body && typeof body === "object" ? [body]
                        : [];
        const talents = raw;
        return talents.find((talent) => {
            const talentIdentity = (0, normalize_1.normalizeIdentity)({
                email: talent.email,
                phone: talent.phone,
                nik: talent.nik,
            ...
            if (field === "nik")
                return Boolean(identity.nik) && talentIdentity.nik === identity.nik;
            if (field === "email")
                return Boolean(identity.email) && talentIdentity.email === identity.email;
            if (field === "phone")
                return Boolean(identity.phone) && talentIdentity.phone === identity.phone;
            return false;
        });
    }
    /**
     ...
     */
    toResult(matched, stafId, matchField, source) {
        if (!matched)
            return newCandidateResult(source);
        return {
            matched: true,
            idrkos_staf_id: stafId,
            status: "idrkos_verified",
            match_field: matchField,
            listing_priority: types_1.LISTING_PRIORITY.idrkos_verified,
            source,
        };
    }
}
exports.IdrkosService = IdrkosService;
