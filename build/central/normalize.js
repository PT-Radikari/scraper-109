...
exports.applicationNaturalKey = exports.jobVacancyNaturalKey = exports.candidateNaturalKey = exports.normalizeIdentity = exports.normalizeNik = exports.normalizeName = exports.normalizePhone = exports.normalizeEmail = void 0;
 ...
 */
function candidateNaturalKey(candidate) {
    const portal = (candidate.source_portal || "unknown").trim().toLowerCase();
    const identity = normalizeIdentity(candidate);
    if (identity.nik)
        return `${portal}:nik:${identity.nik}`;
    if (identity.email)
        return `${portal}:email:${identity.email}`;
    if (identity.phone)
        return `${portal}:phone:${identity.phone}`;
    if (candidate.source_candidate_id) {
        return `${portal}:portal_id:${String(candidate.source_candidate_id).trim()}`;
    }
    if (identity.full_name)
        return `${portal}:name:${identity.full_name}`;
    throw new Error(`Cannot build a candidate natural key without an email, phone, NIK, portal id or name (portal: ${portal})`);
}
exports.candidateNaturalKey = candidateNaturalKey;
/**
 ...
 */
function jobVacancyNaturalKey(vacancy) {
...
exports.jobVacancyNaturalKey = jobVacancyNaturalKey;
/**
 ...
 */
function applicationNaturalKey(application) {
    ...
    const candidateKey = candidateNaturalKey(Object.assign({ source_portal: application.source_portal }, application.candidate));
    const vacancy = (application.source_vacancy_id || normalizeName(application.applied_for) || "unknown").trim();
    return `${portal}:application:${candidateKey}|${vacancy}`;
}
exports.applicationNaturalKey = applicationNaturalKey;
