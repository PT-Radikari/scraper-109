...
app.post("/api/glints/promote", express_1.default.json(), (req, res) => {
    ...
    const name = typeof ((_d = req.body) === null || _d === void 0 ? void 0 : _d.name) === "string" ? req.body.name.replace(/\s+/g, " ").trim() : "";
    ...
    if (((_e = req.body) === null || _e === void 0 ? void 0 : _e.confirm) !== true) {
        res.status(400).json({ error: "confirm must be true: moving applicants to Terhubung cannot be undone by the scraper" });
        return;
    }
    if (((_f = scraperState.glints) === null || _f === void 0 ? void 0 : _f.status) === "running" || scraperState[GLINTS_PROMOTE].status === "running") {
        res.status(409).json({ error: "a Glints run is already in progress" });
        return;
    }
    runScraper(GLINTS_PROMOTE, name ? [GLINTS_PROMOTE, jid !== null && jid !== void 0 ? jid : "-", "1", name] : [GLINTS_PROMOTE, jid !== null && jid !== void 0 ? jid : "-", String(limit)]);
...
app.get("/api/dashboard/candidates", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const pageSize = typeof req.query.size === "string" && /^\d+$/.test(req.query.size) ? Math.min(Math.max(Number(req.query.size), 1), 500) : 100;
        const pageIndex = typeof req.query.page === "string" && /^\d+$/.test(req.query.page) ? Math.min(Number(req.query.page), 1000) : 0;
        ...
        res.json(yield (0, dashboardData_1.getCandidates)(config, Object.assign(Object.assign({ portal,
            search, hasCv: presence(req.query.cv), hasPhone: presence(req.query.phone), hasEmail: presence(req.query.email), vacancyId }, visibility), { limit: pageSize, offset: pageIndex * pageSize })));
    }
    catch (error) {
        dashboardError(res, error);
    }
...
app.post("/api/dashboard/sign-url", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const canonical = typeof portal === "string" ? (0, dashboardData_1.canonicalPortal)(portal) : portal;
    if (typeof canonical !== "string" ||
        !dashboardData_1.ALL_PORTALS.includes(canonical) ||
    ...
    try {
        const result = yield (0, dashboardData_1.getSignedUrl)(config, { portal: canonical, candidateId, kind });
        if (!result) {
