...
- `src/server.ts` loads portal modules and configs **lazily** per dispatched command (`buildPortalRunner`), so one portal's broken dependency cannot crash another portal's container; `tests/serverLazyPortals.test.ts` proves the glints graph never touches pdf-parse/sqlite3/other portals. Keep new portal wiring inside `portalRunnerFactories`, never as top-level imports. Boot prints one `[boot] node ... | playwright ... | command ...` line for runtime-mismatch triage.
