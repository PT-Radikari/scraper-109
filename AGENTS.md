...
- One Dokploy container per portal: `xvfb:<portal>` runs the continuous loop (`<portal>-continuous` in `src/server.ts`, per-cycle `scrape.scrape_runs` row via `runPortalCycle`), `xvfb:<portal>:once` a single run. An expired portal session throws one loud `[PORTAL] Session expired ...` line, fails that cycle, and the loop keeps cycling — never try to refresh sessions in code.
